...
- Источники данных: для MVP/Baseline - любые репозитории компании. Для построения решения было предоставлено 5 репозиториев, на основе которых будет подбираться структура графовой базы и стратегия индексации. Эти репозитории будут использоваться для валидации. Для каждого репозитория были составлены 5–10 вопросов с правильным ответом для дальнейшей валидации и *LLM-as-a-judge*
...
- Инкрементальные обновления по webhook/CI на push/merge
...
- **MVP:** семантические чанки по `function/method/class`, расширенный граф (`calls/inherits/refs`), инкрементальная переиндексация, контроль качества графа/индекса
...
- **Датасет:** для каждого из пяти репозиториев составлены 5–10 тестовых вопросов с указанием правильных фрагментов (где это релевантно). Это позволяет измерять Recall@K, MRR и nDCG.
...
- **Hybrid retrieval:** динамический выбор и комбинирование графовых и векторных результатов
- **Реранкинг:** лёгкий cross-encoder для доранжирования топ-N кандидатов 
- **Расширение графа:** добавление узлов/связей (переменные, namespace и др.)
- **Docstring generation:** для функций без комментариев генерируем краткие описания с помощью LLM, повышая семантическую информативность
- **Автоматизация валидации:** генерируем дополнительные вопросы синтетически, чтобы лучше покрыть разные типы запросов
...
- **Улучшение (MVP):** рост метрик для LLM-as-a-Judge за счёт гибридного retrieval и комбинации источников снижение числа неточных ответов. Проверяем, когда агент выбирает граф, а когда семантику, дополняем правилами промпт, если надо. Дополнительно: улучшение структуры графа и добавление докстрингов.
...
- **Подключение к внутренним системам.** Завершить настройку CI/webhook для автоматической переиндексации, подключить сервис к базовым инструментам (веб-интерфейс)
- **Начало пилотной эксплуатации.** Запустить сервис на ограниченном наборе репозиториев (MVP), настроить ACL, убедиться, что локальная LLM справляется с нагрузкой и выполняет требования по латентности
